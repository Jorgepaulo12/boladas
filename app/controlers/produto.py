...
from models import Produto, Usuario, InfoUsuario,Comentario,produto_likes,Seguidor,Notificacao,Anuncio,Wallet,Status
    ...
    if usuario.conta_pro:
        ...
        if produtos_hoje >= LIMITE_DIARIO:
            ...
            if wallet.saldo_principal >= VALOR_PARA_PUBLICAR:
                ...
                transacao = Transacao(usuario_id=usuario.id, msisdn=usuario_id.username,tipo="saida", valor=VALOR_PARA_PUBLICAR, referencia=data["lacamento de produto"], status="sucesso")
                db.add(transacao)
                db.commit()    
