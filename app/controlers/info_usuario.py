...
from models import InfoUsuario, Notificacao
...
def update_revisao_info_usuario(db_info_usuario, nova_revisao: str, db: Session, motivo: str = None):
    """
    Atualiza o campo 'revisao' do InfoUsuario e cria uma notificação associada ao usuário.
    ...
    """
    # Atualizar o campo 'revisao'
    db_info_usuario.revisao = nova_revisao
    db.commit()  # Salvar a alteração no banco
    db.refresh(db_info_usuario)  # Atualizar a instância do usuário

    # Criar a mensagem da notificação dependendo da revisão
    if nova_revisao == "não":
    ...
    # Criar uma nova notificação para o usuário revisado
    nova_notificacao = Notificacao(
        usuario_id=db_info_usuario.usuario_id,
        mensagem=mensagem,
        data=datetime.utcnow()
    )

    # Adicionar a notificação ao banco de dados
    db.add(nova_notificacao)
    db.commit()

    return {"message": "Campo 'revisao' atualizado com sucesso e notificação enviada", "info_usuario": db_info_usuario}
