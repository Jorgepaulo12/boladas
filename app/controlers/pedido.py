...
def verificar_integridade_saldo(db: Session, user_id: int):
    ...
    saldo_atual = wallet.saldo_principal
    print(f"Saldo atual do usuário {user_id}: {saldo_atual}")

    # Somar todas as transações de entrada (status "sucesso")
    transacoes_entrada = db.query(Transacao).filter(Transacao.usuario_id == user_id, Transacao.status == "sucesso").all()
    saldo_calculado_entrada = sum([Decimal(transacao.valor) for transacao in transacoes_entrada])

    # Somar todas as transações de saída (status "saída")
    transacoes_saida = db.query(Transacao).filter(Transacao.usuario_id == user_id, Transacao.status == "saida").all()
    saldo_calculado_saida = sum([Decimal(transacao.valor) for transacao in transacoes_saida])

    # Calcular o saldo total
    saldo_calculado = saldo_calculado_entrada - saldo_calculado_saida
